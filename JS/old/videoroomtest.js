...
// We make use of this 'server' letiable to provide the address of the
...
// on the 'window.location.protocol' prefix to build the letiable, in
// particular to also change the port used to contact Janus (8088 for
// HTTP and 8089 for HTTPS, if enabled).
// In case you place Janus behind an Apache frontend (as we did on the
// online demos at http://janus.conf.meetecho.com) you can just use a
// relative path for the letiable, e.g.:
//
// 		let server = "/janus";
//
...
if( window.location.protocol === "http:" ) {
	server = "http://" + window.location.hostname + ":9887/janus";
} else {
	server = "https://" + window.location.hostname + ":9876/janus";
}
							...
							onmessage: function( msg, jsep ) {
								...
								if( event ) {
									if( event === "joined" ) {
										...
										//negotiate WebRTC and attach to existing feeds, if any
										myid    = msg[ "id" ];
										...
										if( msg[ "publishers" ] ) {
											...
											if( msg[ "error_code" ] === 426 ) {
												...
												bootbox.alert( "<p>Apparently room <code>" +
														myroom +
	...
	janus.attach({
		...
		onremotestream: function( stream ) {
			...
			if( $( "#remotevideo" + remoteFeed.rfindex ).length === 0 ) {
				addButtons = true;
				// No remote video yet
				$( "#videoremote" + remoteFeed.rfindex ).append( "<video class=\"rounded centered\" id=\"waitingvideo" +
						remoteFeed.rfindex +
						"\" width=\"100%\" height=\"100%\" />" );
				$( "#videoremote" + remoteFeed.rfindex ).append( "<video class=\"rounded centered relative hide\" id=\"remotevideo" +
						remoteFeed.rfindex +
						"\" width=\"100%\" height=\"100%\" autoplay playsinline/>" );
				$( "#videoremote" + remoteFeed.rfindex ).append( "<span class=\"label label-primary hide\" id=\"curres" +
						remoteFeed.rfindex +
			...
			if( !videoTracks || videoTracks.length === 0 ) {
				...
				if( $( "#videoremote" + remoteFeed.rfindex + " .no-video-container" ).length === 0 ) {
					$( "#videoremote" + remoteFeed.rfindex ).append( "<div class=\"no-video-container\">" +
							"<i class=\"fa fa-video-camera fa-5 no-video-icon\"></i>" +
...
function addSimulcastButtons( feed, temporal ) {
	...
	$( "#remote" + index )
		.parent()
		.append( `<div id="simulcast${index}" class="btn-group-vertical btn-group-vertical-xs pull-right">	<div class"row">		<div class="btn-group btn-group-xs" style="width: 100%">			<button id="sl${index}-2" type="button" class="btn btn-primary" data-toggle="tooltip" title="Switch to higher quality" style="width: 33%">SL 2</button>			<button id="sl${index}-1" type="button" class="btn btn-primary" data-toggle="tooltip" title="Switch to normal quality" style="width: 33%">SL 1</button>			<button id="sl${index}-0" type="button" class="btn btn-primary" data-toggle="tooltip" title="Switch to lower quality" style="width: 34%">SL 0</button>		</div>	</div>	<div class"row">		<div class="btn-group btn-group-xs hide" style="width: 100%">			<button id="tl${index}-2" type="button" class="btn btn-primary" data-toggle="tooltip" title="Cap to temporal layer 2" style="width: 34%">TL 2</button>			<button id="tl${index}-1" type="button" class="btn btn-primary" data-toggle="tooltip" title="Cap to temporal layer 1" style="width: 33%">TL 1</button>			<button id="tl${index}-0" type="button" class="btn btn-primary" data-toggle="tooltip" title="Cap to temporal layer 0" style="width: 33%">TL 0</button>		</div>	</div></div>` );
